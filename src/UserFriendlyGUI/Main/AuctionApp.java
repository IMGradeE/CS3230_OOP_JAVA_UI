...
class AuctionItem {
    ...
    /* Since each bidder performs actions on its own thread sequentially, we need not lock the bidder object or worry
     about races on its budget or other members */
    public boolean placeBid(Bidder bidder, double bidAmount) {
        // TODO:
        while(!lock.isHeldByCurrentThread()){
            lock.tryLock();
        }
        double bidderBudget = bidder.getBudget();
        if (bidAmount > highestBid && bidderBudget >= bidAmount) {
            highestBid = bidAmount;
            currentHighestBidder = bidder;
            bidder.setBudget(bidderBudget-bidAmount);
            lock.unlock();
            return true;
        }
        lock.unlock();
